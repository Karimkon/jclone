...
@section('title', 'Add New Service - Vendor Dashboard')
    ...
    <form action="{{ route('vendor.services.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="service_category_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="5" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                              placeholder="Describe your service in detail. What's included? What makes it special?">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="pricing_type" id="pricingType" required 
                        ...
                        <option value="hourly" {{ old('pricing_type') == 'hourly' ? 'selected' : '' }}>Per Hour</option>
                        <option value="starting_from" {{ old('pricing_type') == 'starting_from' ? 'selected' : '' }}>Starting From</option>
                        <option value="negotiable" {{ old('pricing_type') == 'negotiable' ? 'selected' : '' }}>Negotiable</option>
                        <option value="free_quote" {{ old('pricing_type') == 'free_quote' ? 'selected' : '' }}>Free Quote / Contact for Price</option>
                    </select>
                </div>
            ...
            <p class="text-sm text-gray-500 mt-2">
                ...
                Choose how you want to display your pricing to customers.
            </p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="contact_phone" value="{{ old('contact_phone', auth()->user()->vendorProfile->phone ?? '') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                </div>
