...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
    <div class="kpi-card xl:col-span-1">
        ...
        <div class="text-xs text-dark-500 mt-1">From orders</div>
    </div>
    <div class="kpi-card xl:col-span-1">
        <div class="flex items-center gap-3 mb-2">
        ...
        <div class="text-xs text-dark-500 mt-1">8% of order subtotal</div>
    </div>
    <div class="kpi-card xl:col-span-1" style="border-color:rgba(139,92,246,0.3);">
        <div class="flex items-center gap-3 mb-2">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center text-purple-400" style="background:rgba(139,92,246,0.15);"><i class="fas fa-crown"></i></div>
            <div class="text-xs text-dark-400">Subscription Revenue</div>
        </div>
        <div class="text-2xl font-bold text-purple-300 tabular-nums">UGX {{ number_format($subscriptionRevenuePeriod, 2) }}</div>
        <div class="text-xs text-dark-500 mt-1">{{ $subscriptionActiveCount }} active plans · MRR UGX {{ number_format($subscriptionMRR, 0) }}</div>
    </div>
    <div class="kpi-card xl:col-span-1" style="border-color:rgba(234,179,8,0.3);">
        <div class="flex items-center gap-3 mb-2">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center text-yellow-400" style="background:rgba(234,179,8,0.1);"><i class="fas fa-coins"></i></div>
            <div class="text-xs text-dark-400">Total Platform Income</div>
        </div>
        <div class="text-2xl font-bold text-yellow-400 tabular-nums">UGX {{ number_format($periodCommission + $subscriptionRevenuePeriod, 2) }}</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="chart-card" style="border-color:rgba(139,92,246,0.2);">
        <div class="flex items-center gap-2 mb-4">
            <i class="fas fa-crown text-yellow-400"></i>
            <h3 class="text-sm font-semibold text-purple-300">Subscription Revenue by Plan</h3>
        </div>
        @forelse($subscriptionRevenueByPlan as $sp)
        <div class="flex items-center justify-between p-3 rounded-lg mb-2" style="background:rgba(139,92,246,0.07);">
            <div class="flex items-center gap-2">
                <span class="inline-block w-2 h-2 rounded-full
                    @if($sp->slug == 'gold') bg-yellow-400
                    @elseif($sp->slug == 'silver') bg-gray-400
                    @elseif($sp->slug == 'bronze') bg-orange-400
                    @else bg-blue-400
                    @endif"></span>
                <span class="text-sm text-dark-200 font-medium">{{ $sp->name }}</span>
            </div>
            <div class="text-right">
                <div class="text-sm font-bold text-purple-300 tabular-nums">UGX {{ number_format($sp->total, 0) }}</div>
                <div class="text-xs text-dark-500">{{ $sp->count }} payments</div>
            </div>
        </div>
        @empty
        <div class="text-center text-dark-500 py-8">No subscription payments in this period</div>
        @endforelse
        @if($subscriptionRevenueByPlan->count() > 0)
        <div class="flex items-center justify-between p-3 rounded-lg mt-2" style="background:rgba(139,92,246,0.12); border:1px solid rgba(139,92,246,0.2);">
            <span class="text-sm font-bold text-dark-200">Total</span>
            <span class="text-sm font-bold text-purple-300 tabular-nums">UGX {{ number_format($subscriptionRevenueByPlan->sum('total'), 0) }}</span>
        </div>
        @endif
    </div>
    ...
    <div class="chart-card" style="border-color:rgba(139,92,246,0.2);">
        <div class="flex items-center gap-2 mb-4">
            ...
            <h3 class="text-sm font-semibold text-purple-300">Monthly Subscription Revenue (12 Months)</h3>
        </div>
        <div style="position:relative;height:200px;"><canvas id="subscriptionTrendChart"></canvas></div>
        @if($monthlySubscriptionRevenue->count() > 0)
        <div class="mt-3 grid grid-cols-2 gap-2">
            <div class="text-center p-2 rounded" style="background:rgba(139,92,246,0.08);">
                <div class="text-sm font-bold text-purple-300">{{ $subscriptionActiveCount }}</div>
                <div class="text-xs text-dark-500">Active Subscriptions</div>
            </div>
            <div class="text-center p-2 rounded" style="background:rgba(139,92,246,0.08);">
                <div class="text-sm font-bold text-purple-300">UGX {{ number_format($subscriptionMRR, 0) }}</div>
                <div class="text-xs text-dark-500">Monthly Recurring Revenue</div>
            </div>
        </div>
        @endif
    </div>
...
<div class="chart-card">
    ...
    <div class="overflow-x-auto">
        <table class="data-table">
            ...
            <tbody>
                @php
                    // Index subscription data by month for quick lookup
                    $subByMonth = $monthlySubscriptionRevenue->keyBy('month');
                @endphp
                @foreach($monthlyFinancials as $mf)
                @php $subRev = $subByMonth->get($mf->month)?->revenue ?? 0; @endphp
                <tr>
                    <td>{{ $mf->month }}</td>
                    <td class="tabular-nums">UGX {{ number_format($mf->revenue, 2) }}</td>
                    <td class="tabular-nums text-indigo-400">UGX {{ number_format($mf->commission, 2) }}</td>
                    <td class="tabular-nums text-purple-400">UGX {{ number_format($subRev, 2) }}</td>
                    <td class="tabular-nums text-yellow-400 font-semibold">UGX {{ number_format($mf->commission + $subRev, 2) }}</td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const subData = @json($monthlySubscriptionRevenue);
    if (subData.length > 0 && document.getElementById('subscriptionTrendChart')) {
        new Chart(document.getElementById('subscriptionTrendChart'), {
            type: 'bar',
            data: {
                labels: subData.map(d => d.month),
                datasets: [{
                    label: 'Subscription Revenue (UGX)',
                    data: subData.map(d => parseFloat(d.revenue)),
            ...
            options: {
                ...
                scales: {
                    y: { beginAtZero: true, grid: { color: 'rgba(139,92,246,0.05)' }, ticks: { callback: v => 'UGX ' + v.toLocaleString() } },
                    x: { grid: { color: 'rgba(139,92,246,0.05)' } }
                }
            }
        });
    }
