...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('buyer.orders.place-order') }}" method="POST" id="checkoutForm">
        ...
        {{-- Pass selected item keys as hidden inputs --}}
        @if(isset($selectedKeys) && !empty($selectedKeys))
                        ...
                        <label class="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-indigo-500 transition payment-method">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center justify-between">
                                    ...
                                    @if($wallet->available_balance < ($subtotal + $taxTotal))
                ...
                <div class="bg-white rounded-lg shadow p-6 sticky top-4">
                    ...
                    <div class="space-y-3 mb-4 max-h-64 overflow-y-auto">
                        ...
                        <div class="flex items-center space-x-3 pb-3 border-b">
                            ...
                            <p class="font-semibold text-sm">UGX {{ number_format(($item['unit_price'] ?? 0) * ($item['quantity'] ?? 1), 0) }}</p>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-2 mb-4 pt-4 border-t">
                        <div class="flex justify-between text-gray-600">
                            <span>Subtotal ({{ count($selectedItems) }} items)</span>
                            <span>UGX {{ number_format($subtotal, 0) }}</span>
                        </div>
                        @if($taxTotal > 0)
                        <div class="flex justify-between text-gray-600">
                            <span>Import/Tax Charges</span>
                            <span>UGX {{ number_format($taxTotal, 0) }}</span>
                        </div>
                        @endif
                        <div class="flex justify-between font-bold text-lg pt-2 border-t">
                            <span>Total</span>
                            <span class="text-indigo-600">UGX {{ number_format($subtotal + $taxTotal, 0) }}</span>
                        </div>
                    </div>
