...
<!-- Wallet Payment Option - SIMPLE FORM -->
<div class="border-2 border-green-500 rounded-lg p-6 mb-4">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center">
            ...
            <div>
                <h3 class="font-bold text-gray-800">Pay with Wallet</h3>
                <p class="text-gray-600">Instant payment from your balance</p>
            </div>
        </div>
        <div class="text-right">
            <p class="text-xl font-bold text-green-600">UGX {{ number_format($walletBalance ?? 0, 0) }}</p>
    ...
    <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
        <div class="flex items-center justify-between mb-2">
            ...
            <span class="font-bold">UGX {{ number_format($order->total, 0) }}</span>
        </div>
        <div class="flex items-center justify-between">
            <span class="text-gray-600">New Balance:</span>
            <span class="font-bold text-green-600">
                UGX {{ number_format(($walletBalance ?? 0) - $order->total, 0) }}
            </span>
        </div>
    </div>
    
    <!-- Simple Form -->
    <form method="POST" action="{{ route('buyer.orders.pay-with-wallet', $order->id) }}" 
          onsubmit="return confirm('Pay UGX {{ number_format($order->total, 0) }} from your wallet?')">
        @csrf
        ...
        <button type="submit" 
            ...
            Pay UGX {{ number_format($order->total, 0) }} Now
        </button>
    </form>
    
    <p class="text-xs text-gray-500 text-center mt-3">
        <i class="fas fa-shield-alt mr-1"></i> Secure · Instant · No fees
    </p>
</div>
@else
<!-- Show "Add Funds" option if insufficient balance -->
<div class="border-2 border-yellow-200 rounded-lg p-6 mb-4">
    <div class="flex items-center mb-4">
        <div class="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center mr-4">
            <i class="fas fa-exclamation-triangle text-2xl text-yellow-600"></i>
        </div>
        <div>
            <h3 class="font-bold text-gray-800">Insufficient Wallet Balance</h3>
            <p class="text-gray-600">Add funds to pay with wallet</p>
        </div>
    </div>
    
    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
        <div class="text-center">
            <p class="text-yellow-700 font-medium mb-1">
                Need UGX {{ number_format($order->total - ($walletBalance ?? 0), 0) }} more
            </p>
            <p class="text-sm text-yellow-600">
                Current balance: UGX {{ number_format($walletBalance ?? 0, 0) }}
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <a href="{{ route('buyer.wallet.index') }}" 
           class="bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition flex items-center justify-center">
            <i class="fas fa-wallet mr-2"></i>
            View Wallet
        </a>
        <a href="{{ route('buyer.wallet.deposit') }}" 
           class="bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition flex items-center justify-center">
